...
import { Lock, Download, Users, Camera, Briefcase, Star, LogOut, Eye, EyeOff, Trash2, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        setError(`Server error: ${json.error || 'Unknown error'}`);
      } else {
        setData(json.data || []);
        setLoggedIn(true);
      }
  ...
  const downloadCSV = () => {
    const headers = ['Date & Time', 'Name', 'Organization', 'Email', 'Mobile', 'Theme', 'Type', 'Career Style', 'Photos', 'Photo URL'];
    const rows = visitors.map(r => [
  ...
  const handleDelete = async (email: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setData(prev => prev.filter(r => r.email !== email));
      }
    } finally {
  ...
  // Group by email — one entry per unique visitor, all photos collected
  const visitorMap = new Map<string, Row & { photoCount: number; allPhotos: { url: string; theme: string; theme_type: string; created_at: string }[] }>();
  for (const row of [...data].reverse()) {
    const key = row.email;
    if (!visitorMap.has(key)) {
      visitorMap.set(key, { ...row, photoCount: 0, allPhotos: [] });
    }
    const v = visitorMap.get(key)!;
    v.photoCount += 1;
    if (row.transformed_photo_url && !row.transformed_photo_url.startsWith('data:')) {
      v.allPhotos.push({ url: row.transformed_photo_url, theme: row.selected_theme, theme_type: row.theme_type, created_at: row.created_at });
    }
  }
  ...
  const totalFun = visitors.filter(r => r.theme_type === 'fun').length;
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <header className="border-b border-white/10 px-6 py-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-xs mt-0.5">Gemini Magic Booth — All leads</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => { setLoggedIn(false); setPassword(''); setData([]); }}
      ...
      <div className="grid grid-cols-3 gap-4 px-6 py-5">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-gray-500 text-xs">Total Visitors</div>
          </div>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center gap-4">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10 bg-white/5">
                    ...
                    <th className="text-left px-4 py-3 text-gray-400 font-semibold">Mobile</th>
                    ...
                    <th className="text-left px-4 py-3 text-gray-400 font-semibold">Latest</th>
                  ...
                  {visitors.map((row, i) => (
                    <>
                      <tr key={row.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                        ...
                        <td className="px-4 py-3 text-gray-300">{row.mobile_number}</td>
                        ...
                        <td className="px-4 py-3">
                          {row.allPhotos.length > 0 ? (
                            <a href={row.allPhotos[0].url} target="_blank" rel="noopener noreferrer"
                      ...
                      {expandedEmail === row.email && row.allPhotos.length > 1 && (
                        <tr key={`${row.id}-expanded`} className="bg-white/3 border-b border-white/5">
                          <td colSpan={10} className="px-6 py-4">
                            <p className="text-gray-500 text-xs mb-3">All images for {row.name}</p>
                            <div className="flex flex-wrap gap-3">
                              {row.allPhotos.map((photo, idx) => (
                                <a key={idx} href={photo.url} target="_blank" rel="noopener noreferrer" className="group relative">
                                  <img src={photo.url} alt={photo.theme} className="w-20 h-20 object-cover rounded-xl border border-white/10 group-hover:border-purple-400/50 transition-colors" />
                                  <div className="mt-1 text-center">
                                    <span className={`text-xs px-1.5 py-0.5 rounded-full ${photo.theme_type === 'career' ? 'text-sky-400' : 'text-violet-400'}`}>
                                      {photo.theme}
                                    </span>
                                  </div>
                                </a>
